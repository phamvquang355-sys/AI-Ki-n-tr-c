...
const resizeAndCompressImage = async (
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      console.log(
        `[Image Process] Resized to ${width}x${height}. Old size: ~${Math.round(fileData.base64.length / 1024)}KB. New size: ~${Math.round(compressedDataUrl.length / 1024)}KB`,
      );
...
export const generateVideoExternal = async (
  ...
  if (!triggerRes.ok) {
    ...
    if (triggerRes.status === 413) {
      errMsg =
        "Ảnh tải lên quá lớn. Hệ thống đã thử nén nhưng vẫn vượt quá giới hạn.";
    }

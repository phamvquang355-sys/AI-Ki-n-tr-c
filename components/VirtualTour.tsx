...
const OrbitRightIcon = () => (
  ...
  >
    <path
      strokeLinecap="round"
      strokeLinejoin="round"
      d="M13 15l3-3m0 0l-3-3m3 3H5a5 5 0 00-5 5v1"
    />
  </svg>
);
const ZoomInIcon = () => (
...
const ResetIcon = () => (
  ...
  >
    <path
      strokeLinecap="round"
      strokeLinejoin="round"
      d="M4 4v5h5M20 12A8 8 0 1012 4v4"
    />
  </svg>
);
...
const VirtualTour: React.FC<VirtualTourProps> = ({ state, onStateChange }) => {
  ...
  const handleTourStep = async (
    ...
    const baseInstruction =
      "Generate a new image from the camera's new position. Maintain the exact same architectural style, materials, lighting, and overall atmosphere from the source image. The change should only be the camera's perspective. The final image must look photorealistic.";

    switch (action) {
      case "pan-left":
        prompt = `From the current camera position, pan the camera to the left by ${tourStepSize} degrees. ${baseInstruction}`;
        break;
      case "pan-right":
        prompt = `From the current camera position, pan the camera to the right by ${tourStepSize} degrees. ${baseInstruction}`;
        break;
      case "tilt-up":
        prompt = `From the current camera position, tilt the camera upwards by ${tourStepSize} degrees. ${baseInstruction}`;
        break;
      case "tilt-down":
        prompt = `From the current camera position, tilt the camera downwards by ${tourStepSize} degrees. ${baseInstruction}`;
        break;
      case "orbit-left":
        prompt = `From the current camera position, orbit the camera to the left around the central subject by ${tourStepSize} degrees. ${baseInstruction}`;
        break;
      case "orbit-right":
        prompt = `From the current camera position, orbit the camera to the right around the central subject by ${tourStepSize} degrees. ${baseInstruction}`;
        break;
      case "zoom-in":
        prompt = `From the current camera position, zoom in, moving the camera forward. The field of view should narrow. ${baseInstruction}`;
        break;
      case "zoom-out":
        prompt = `From the current camera position, zoom out, moving the camera backward. The field of view should widen. ${baseInstruction}`;
        break;
      default:
    ...
    <div className="flex flex-col gap-8">
      <div>
        ...
        <p className="text-text-secondary dark:text-gray-300 mb-6">
          Tải lên một ảnh phối cảnh, sau đó sử dụng các nút điều hướng để "di
          chuyển" xung quanh không gian. AI sẽ tạo ra các góc nhìn mới cho bạn.
        </p>
      </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-main-bg/50 dark:bg-dark-bg/50 p-6 rounded-xl border border-border-color dark:border-gray-700">
            <label className="block text-sm font-medium text-text-secondary dark:text-gray-400 mb-2">
              1. Tải Lên Ảnh Bắt Đầu
            </label>
            <ImageUpload
              onFileSelect={handleTourFileSelect}
              previewUrl={sourceImage?.objectURL}
            />
          </div>
          <div className="bg-main-bg/50 dark:bg-dark-bg/50 p-6 rounded-xl border border-border-color dark:border-gray-700">
            <label
              htmlFor="tour-step-size"
              className="block text-sm font-medium text-text-secondary dark:text-gray-400"
            >
              2. Điều chỉnh bước di chuyển
            </label>
            <div className="flex items-center gap-4 mt-2">
              <input
                ...
                value={tourStepSize}
                onChange={(e) =>
                  onStateChange({ tourStepSize: Number(e.target.value) })
                }
                className="w-full h-2 bg-gray-200 dark:bg-gray-700 rounded-lg appearance-none cursor-pointer accent-accent"
                disabled={isLoading}
              />
              <span className="font-semibold text-text-primary dark:text-white w-12 text-center">
                {tourStepSize}°
              </span>
            </div>
          </div>
          ...
          <div className="lg:col-span-2">
            ...
            <div className="mt-6 bg-main-bg/50 dark:bg-dark-bg/50 p-4 rounded-xl border border-border-color dark:border-gray-700 flex justify-center items-center gap-2">
              <button
                title="Quay trái"
                onClick={() => handleTourStep("orbit-left")}
                disabled={!currentTourImage || isLoading}
                className="p-3 rounded-full bg-surface dark:bg-gray-700 hover:bg-accent/20 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <OrbitLeftIcon />
              </button>
              <div className="grid grid-cols-3 gap-2">
                ...
                <button
                  title="Nhìn lên"
                  onClick={() => handleTourStep("tilt-up")}
                  disabled={!currentTourImage || isLoading}
                  className="p-3 rounded-full bg-surface dark:bg-gray-700 hover:bg-accent/20 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <PanUpIcon />
                ...
                <button
                  title="Lia trái"
                  onClick={() => handleTourStep("pan-left")}
                  disabled={!currentTourImage || isLoading}
                  className="p-3 rounded-full bg-surface dark:bg-gray-700 hover:bg-accent/20 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <PanLeftIcon />
                ...
                <div className="flex flex-col gap-2">
                  <button
                    title="Phóng to"
                    onClick={() => handleTourStep("zoom-in")}
                    disabled={!currentTourImage || isLoading}
                    className="p-3 rounded-full bg-surface dark:bg-gray-700 hover:bg-accent/20 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <ZoomInIcon />
                  ...
                  <button
                    title="Thu nhỏ"
                    onClick={() => handleTourStep("zoom-out")}
                    disabled={!currentTourImage || isLoading}
                    className="p-3 rounded-full bg-surface dark:bg-gray-700 hover:bg-accent/20 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <ZoomOutIcon />
                ...
                <button
                  title="Lia phải"
                  onClick={() => handleTourStep("pan-right")}
                  disabled={!currentTourImage || isLoading}
                  className="p-3 rounded-full bg-surface dark:bg-gray-700 hover:bg-accent/20 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <PanRightIcon />
                ...
                <button
                  title="Nhìn xuống"
                  onClick={() => handleTourStep("tilt-down")}
                  disabled={!currentTourImage || isLoading}
                  className="p-3 rounded-full bg-surface dark:bg-gray-700 hover:bg-accent/20 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <PanDownIcon />
              ...
              <button
                title="Quay phải"
                onClick={() => handleTourStep("orbit-right")}
                disabled={!currentTourImage || isLoading}
                className="p-3 rounded-full bg-surface dark:bg-gray-700 hover:bg-accent/20 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <OrbitRightIcon />

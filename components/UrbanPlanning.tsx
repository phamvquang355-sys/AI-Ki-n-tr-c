...
const viewTypeOptions = [
  ...
  {
    value: "phối cảnh mắt chim (bird's-eye view)",
    label: "Phối cảnh mắt chim",
  },
  {
    value: "phối cảnh từ trên cao góc 45 độ (aerial 45-degree view)",
    label: "Phối cảnh 45°",
  },
  {
    value: "phối cảnh tầm mắt người đi bộ (street-level perspective)",
    label: "Góc nhìn người",
  },
  {
    value: "phối cảnh ven sông/ven biển (waterfront view)",
    label: "Ven sông/biển",
  },
];
...
const UrbanPlanning: React.FC<UrbanPlanningProps> = ({
  state,
  onStateChange,
  onSendToViewSync,
}) => {
  ...
  const updatePrompt = useCallback(
    (
      type: "viewType" | "density" | "lighting",
      newValue: string,
      oldValue: string,
    ) => {
      const getPromptPart = (partType: string, value: string): string => {
        ...
        switch (partType) {
          case "viewType":
            return `tạo ra một ${value}`;
          case "density":
            return `mô phỏng một ${value}`;
          case "lighting":
            return `với ánh sáng ${value}`;
          default:
            return "";
        }
      };
  ...
  const handleUpscale = async () => {
    ...
    try {
      ...
      const upscalePrompt =
        "Upscale this urban planning rendering to a high resolution. Enhance details like building facades, landscapes, textures, and lighting to make it look photorealistic and professional. Do not change the composition or the core design.";
    ...
    <div className="flex flex-col gap-8">
      ...
      <div>
        <h2 className="text-2xl font-bold text-text-primary dark:text-white mb-4">
          AI Render Quy Hoạch Đô Thị
        </h2>
        <p className="text-text-secondary dark:text-gray-300 mb-6">
          Tạo phối cảnh tổng thể cho các dự án quy hoạch, khu đô thị, hoặc cảnh
          quan lớn từ bản vẽ 2D hoặc mô tả.
        </p>
            ...
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-medium text-text-secondary dark:text-gray-400 mb-2">
                  1. Tải Lên Bản Vẽ/Ảnh Hiện Trạng (Tùy chọn)
                </label>
                <ImageUpload
            ...
            <div className="space-y-4 flex flex-col">
              <div>
                ...
                >
                  ...
                  <span>
                    {isGeneratingPrompt ? "Đang tạo..." : "Tạo tự động Prompt"}
                  </span>
                </button>
              </div>
            ...
            <div className="flex items-center gap-2">
              {resultImages.length === 1 && !upscaledImage && (
                ...
                >
                  ...
                  <span>
                    {isUpscaling ? "Đang nâng cấp..." : "Nâng cấp chi tiết"}
                  </span>
                </button>
              )}
            ...
            {!isLoading && resultImages.length === 0 && (
              <p className="text-text-secondary dark:text-gray-400 p-4 text-center">
                {sourceImage
                  ...
                  : "Nhập mô tả hoặc tải ảnh lên để bắt đầu"}
              </p>
            )}
          </div>
        </div>

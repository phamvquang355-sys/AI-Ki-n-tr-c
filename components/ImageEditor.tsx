  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (false || false || false) {
        ...
        if (maskImage) {
          finalPrompt +=
            " Note: The user provided a mask for specific edits, but in High Quality mode, please perform the requested edit while maintaining the overall composition and integrity of the image as best as possible.";
        }
      ...
      // Standard (Flash) Logic
        ...
        if (referenceImages && referenceImages.length > 0) {
          if (maskImage) {
            results =
              await geminiService.editImageWithMaskAndMultipleReferences(
                prompt,
                sourceImage,
                maskImage,
                referenceImages,
            ...
            results = await geminiService.editImageWithMultipleReferences(
              prompt,
              sourceImage,
              referenceImages,
              numberOfImages,
  ...
  return (
    <div>
      ...
      <p className="text-text-secondary dark:text-gray-300 mb-6">
        Tải lên một bức ảnh và mô tả những thay đổi bạn muốn. Bạn cũng có thể
        dùng công cụ mask để chỉ định vùng cần chỉnh sửa.
      </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-6 flex flex-col">
              ...
              <div className="mt-4">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    title={
                      "Standard" !== "Standard"
                        ? "Chế độ chất lượng cao sẽ tự động xử lý toàn bộ ảnh"
                        : "Vẽ vùng chọn"
                    }
                  >
                    <svg
                ...
                {maskImage && (
                  <p className="text-xs text-green-500 dark:text-green-400 mt-2">
                    Đã áp dụng vùng chọn. AI sẽ chỉ chỉnh sửa trong vùng này.
                  </p>
                )}
                {"Standard" !== "Standard" && (
                  <p className="text-xs text-yellow-500 mt-1">
                    Lưu ý: Chế độ 2K/4K sẽ xử lý toàn bộ ảnh để đảm bảo chất
                    lượng đồng nhất.
                  </p>
                )}
              </div>
          ...
          <div className="bg-main-bg/50 dark:bg-dark-bg/50 p-6 rounded-xl border border-border-color dark:border-gray-700">
            ...
            <p className="text-xs text-text-secondary dark:text-gray-500 mt-2">
              Tải lên tối đa 3 ảnh để AI tham khảo phong cách hoặc chi tiết.
            </p>
          </div>
        </div>
        ...
        <div className="space-y-6 flex flex-col h-full">
          ...
          <div className="flex items-center justify-between bg-gray-100 dark:bg-gray-800/50 rounded-lg px-4 py-2 mb-1 border border-gray-200 dark:border-gray-700">
            ...
            {error && (
              <div className="mt-2 p-3 bg-red-100 border border-red-400 text-red-700 dark:bg-red-900/50 dark:border-red-500 dark:text-red-300 rounded-lg text-sm">
                {error}
              </div>
            )}
          </div>
          ...
          <div className="w-full aspect-video bg-main-bg dark:bg-gray-800/50 rounded-lg border-2 border-dashed border-border-color dark:border-gray-700 flex items-center justify-center overflow-hidden">
            ...
            {!isLoading && resultImages.length === 0 && (
              <p className="text-text-secondary dark:text-gray-400 text-center p-4">
                {sourceImage
                  ? "Kết quả chỉnh sửa sẽ hiển thị ở đây."

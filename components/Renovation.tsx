...
const Renovation: React.FC<RenovationProps> = ({ state, onStateChange }) => {
  ...
  const handleAutoPrompt = async () => {
    ...
    try {
      const generatedPrompt =
        await geminiService.generatePromptFromImageAndText(sourceImage, prompt);
  ...
  const handleGenerate = async () => {
    ...
    if (!sourceImage) {
      onStateChange({
        error: "Vui lòng tải lên một hình ảnh thực tế để bắt đầu.",
      });
      return;
    }
    ...
    try {
      ...
      if (false || false || false) {
        if (maskImage)
          finalPrompt +=
            " Note: The user wanted to mask specific areas, but in High Quality mode, the entire image will be regenerated to match the renovation request while preserving the overall composition.";
        if (referenceImage)
          finalPrompt +=
            " Also, take aesthetic inspiration (colors, materials, atmosphere) from the provided reference image.";
      ...
      // Standard (Flash) Logic
        ...
        if (maskImage && referenceImage) {
          ...
          results = await geminiService.editImageWithMaskAndReference(
            finalPrompt,
            sourceImage,
            maskImage,
            referenceImage,
          ...
          results = await geminiService.editImageWithMask(
            finalPrompt,
            sourceImage,
            maskImage,
            numberOfImages,
    ...
    <div className="flex flex-col gap-8">
      ...
      <div>
        ...
        <p className="text-text-secondary dark:text-gray-300 mb-6">
          Tải lên ảnh chụp thực tế của một công trình hoặc không gian nội thất.
          AI sẽ giúp bạn hình dung phương án cải tạo mới một cách trực quan.
        </p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
                ...
                {sourceImage && (
                  <div className="mt-4">
                    <p className="text-sm text-text-secondary dark:text-gray-400 mb-2">
                      Chỉ định vùng cần cải tạo (tùy chọn):
                    </p>
                    <div className="flex gap-2">
                      <button
                        ...
                        title={
                          "Standard" !== "Standard"
                            ? "Chế độ chất lượng cao sẽ tự động xử lý toàn bộ ảnh để đảm bảo sự đồng nhất"
                            : "Vẽ vùng chọn"
                        }
                      >
                        <svg
                    ...
                    {maskImage && (
                      <p className="text-xs text-green-500 dark:text-green-400 mt-2">
                        Đã áp dụng vùng chọn.
                      </p>
                    )}
                    {"Standard" !== "Standard" && (
                      <p className="text-xs text-yellow-500 mt-1">
                        Lưu ý: Chế độ 2K/4K sẽ cải tạo toàn bộ ảnh để đạt chất
                        lượng tốt nhất.
                      </p>
                    )}
                  </div>
            ...
            <div className="space-y-4 flex flex-col h-full">
              <div>
                ...
                <div className="mt-3">
                  <label
                    htmlFor="renovation-suggestions"
                    className="block text-sm font-medium text-text-secondary dark:text-gray-400 mb-2"
                  >
                    Thêm gợi ý nhanh
                  </label>
        ...
        <div className="w-full aspect-video bg-main-bg dark:bg-gray-800/50 rounded-lg border-2 border-dashed border-border-color dark:border-gray-700 flex items-center justify-center overflow-hidden">
          ...
          {!isLoading && renovatedImages.length === 0 && (
            <p className="text-text-secondary dark:text-gray-400 text-center p-4">
              {sourceImage
                ? "Phương án cải tạo sẽ được hiển thị ở đây."

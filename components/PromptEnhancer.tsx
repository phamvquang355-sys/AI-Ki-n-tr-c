...
const PromptEnhancer: React.FC<PromptEnhancerProps> = ({
  ...
  const handleGenerate = async () => {
    if (!customNeeds.trim() && !sourceImage) {
      onStateChange({
        error: "Vui lòng mô tả yêu cầu của bạn hoặc tải lên một hình ảnh.",
      });
      return;
    }
    ...
    try {
      const result = await geminiService.enhancePrompt(
        customNeeds,
        sourceImage,
      );
    ...
    <div className="flex flex-col gap-8">
      <div>
        ...
        <p className="text-text-secondary dark:text-gray-300 mb-6">
          Cung cấp ý tưởng, từ khóa hoặc hình ảnh, AI sẽ giúp bạn viết một
          prompt chi tiết và chuyên nghiệp để tạo ra những hình ảnh kiến trúc ấn
          tượng.
        </p>
      </div>
        ...
        <div className="space-y-6 flex flex-col">
          <div className="bg-main-bg/50 dark:bg-dark-bg/50 p-6 rounded-xl border border-border-color dark:border-gray-700">
            <label className="block text-sm font-medium text-text-secondary dark:text-gray-400 mb-2">
              1. Tải Lên Ảnh Tham Khảo (Tùy chọn)
            </label>
            <ImageUpload

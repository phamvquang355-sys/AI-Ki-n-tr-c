    ...
    >
      ...
      {isOpen && (
        ...
        >
          ...
          `}</style>
          <div
            className={`max-h-64 overflow-y-auto p-2 scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-600 ${variant === "grid" ? "grid grid-cols-2 gap-2" : "flex flex-col gap-1"}`}
          >
            {options.map((option) => (
              <button
                key={option.value}

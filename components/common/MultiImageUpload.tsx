...
const PlusIcon = () => (
  <svg
    ...
    className="h-8 w-8 text-text-secondary dark:text-gray-400 group-hover:text-accent"
    fill="none"
    viewBox="0 0 24 24"
    stroke="currentColor"
    strokeWidth={2}
...
const MultiImageUpload: React.FC<MultiImageUploadProps> = ({
  ...
  const handleRemove = (objectURLToRemove: string) => {
    const updatedFiles = files.filter(
      (file) => file.objectURL !== objectURLToRemove,
    );
    setFiles(updatedFiles);
    onFilesChange(updatedFiles);
  };
    ...
    <div>
      <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
        {files.map((file) => (
          <div
            key={file.objectURL}
            className="relative group aspect-square bg-main-bg dark:bg-gray-800 rounded-md overflow-hidden border border-border-color dark:border-gray-700"
          >
            <img
              src={file.objectURL}
              alt="Preview"
        ...
        {files.length < maxFiles && (
          ...
          >
            <PlusIcon />
            <p className="text-xs text-text-secondary dark:text-gray-400 mt-1 group-hover:text-accent">
              Thêm ảnh
            </p>
          </div>
        )}

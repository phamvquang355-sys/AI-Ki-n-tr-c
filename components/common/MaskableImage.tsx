...
const MaskableImage: React.FC<MaskableImageProps> = ({
  image,
  onMaskChange,
  maskColor,
}) => {
  ...
  const draw = (event: MouseEvent | TouchEvent) => {
    ...
    ctx.fillStyle =
      mode === "draw" ? maskColor || "rgba(255, 255, 255, 0.7)" : "#000";
    ctx.beginPath();
    ...
    <div className="space-y-4">
        ...
        {isCursorVisible && (
          <div
            ...
            style={{
              ...
              height: brushSize,
              borderColor:
                mode === "draw" ? (maskColor ? "#ef4444" : "white") : "#ef4444",
              backgroundColor:
                mode === "draw"
                  ? maskColor
                    ? "rgba(239, 68, 68, 0.3)"
      ...
      <div className="bg-main-bg dark:bg-gray-800 p-3 rounded-lg border border-border-color dark:border-gray-700 flex flex-col sm:flex-row items-center gap-4">
        ...
        <div className="flex items-center gap-3 flex-grow w-full sm:w-auto">
          <label
            htmlFor="brush-size"
            className="text-sm text-text-secondary dark:text-gray-300 whitespace-nowrap"
          >
            Cỡ bút:
          </label>
          <input

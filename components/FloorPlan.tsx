...
const FloorPlan: React.FC<FloorPlanProps> = ({ state, onStateChange }) => {
  ...
  const handleAutoPrompt = async () => {
    if (!sourceImage) {
      onStateChange({
        error: "Vui lòng tải ảnh mặt bằng lên trước khi tạo prompt tự động.",
      });
      return;
    }
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (renderMode === "top-down") {
        if (!prompt) {
          onStateChange({
            error: "Vui lòng nhập mô tả phong cách.",
            isLoading: false,
          });
          return;
        }
        ...
        if (!layoutPrompt?.trim()) {
          onStateChange({
            error: "Vui lòng nhập mô tả chi tiết về góc nhìn và phong cách.",
            isLoading: false,
          });
          return;
        }
    ...
    <div className="flex flex-col gap-8">
      ...
      <div>
        <h2 className="text-2xl font-bold text-text-primary dark:text-white mb-4">
          AI Render Mặt Bằng (Nội thất & Kiến trúc)
        </h2>
        <p className="text-text-secondary dark:text-gray-300 mb-6">
          Tải lên bản vẽ mặt bằng 2D, AI sẽ biến nó thành phối cảnh 3D cho nội
          thất hoặc kiến trúc theo yêu cầu của bạn.
        </p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
            ...
            <div className="space-y-4 flex flex-col h-full">
              ...
              {renderMode === "top-down" && (
                <div>
                  <label
                    htmlFor="prompt-floorplan"
                    className="block text-sm font-medium text-text-secondary dark:text-gray-400 mb-2"
                  >
                    4.{" "}
                    {planType === "interior"
                      ? "Mô tả phong cách nội thất & vật liệu"
                      : "Mô tả phong cách kiến trúc, mái, sân vườn"}
                  </label>
                  <textarea
                    id="prompt-floorplan"
                    rows={3}
                    ...
                    placeholder={
                      planType === "interior"
                        ? "VD: Phong cách hiện đại, tông màu xám-trắng, sàn gỗ, nội thất tối giản..."
                        : "VD: Phong cách hiện đại, mái thái, tường sơn trắng, có sân vườn nhỏ..."
                    }
                    value={prompt}
              ...
              {renderMode === "perspective" && (
                <div className="space-y-4">
                  <div>
                    <label
                      htmlFor="layout-prompt-floorplan"
                      className="block text-sm font-medium text-text-secondary dark:text-gray-400 mb-2"
                    >
                      4.{" "}
                      {planType === "interior"
                        ? "Mô tả chi tiết góc nhìn và phong cách"
                        : "Mô tả góc nhìn ngoại thất và bối cảnh"}
                    </label>
                    <textarea
                      ...
                      placeholder={
                        planType === "interior"
                          ? "VD: Góc nhìn từ sofa phòng khách, hướng ra cửa sổ lớn. Phong cách nội thất hiện đại..."
                          : "VD: Góc nhìn từ cổng vào, thời tiết nắng đẹp, có cây xanh hai bên. Phong cách kiến trúc nhiệt đới."
                      }
                      value={layoutPrompt}
                      onChange={(e) =>
                        onStateChange({ layoutPrompt: e.target.value })
                      }
                      disabled={isLoading}
                    />
                    <button
                  ...
                  <div>
                    <label className="block text-sm font-medium text-text-secondary dark:text-gray-400 mb-2">
                      5. Tải Ảnh Tham Chiếu Phong Cách (Tùy chọn)
                    </label>
                    <ImageUpload
        ...
        <div className="w-full aspect-video bg-main-bg dark:bg-gray-800/50 rounded-lg border-2 border-dashed border-border-color dark:border-gray-700 flex items-center justify-center overflow-hidden">
          ...
          {!isLoading && resultImages.length === 0 && (
            <p className="text-text-secondary dark:text-gray-400 text-center p-4">
              {sourceImage
                ? "Kết quả render 3D sẽ được hiển thị ở đây."
                : "Tải lên một mặt bằng để bắt đầu."}
            </p>
          )}
        </div>
      </div>

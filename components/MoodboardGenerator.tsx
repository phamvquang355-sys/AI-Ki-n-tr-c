  ...
  const handleAutoPrompt = async () => {
    ...
    try {
      ...
      if (mode === "moodboardToScene") {
        generatedPrompt = await geminiService.generatePromptFromImageAndText(
          sourceImage,
          prompt,
        );
      } else {
        // 'sceneToMoodboard'
        generatedPrompt =
          await geminiService.generateMoodboardPromptFromScene(sourceImage);
      }
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (mode === "moodboardToScene") {
        if (!prompt) {
          onStateChange({
            error: "Vui lòng mô tả loại không gian bạn muốn tạo.",
            isLoading: false,
          });
          return;
        }
    ...
    <div className="flex flex-col gap-8">
      ...
      <div>
        <h2 className="text-2xl font-bold text-text-primary dark:text-white mb-4">
          {mode === "moodboardToScene"
            ? "AI Tạo Ảnh từ Moodboard"
            : "AI Tạo Moodboard từ Ảnh"}
        </h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-medium text-text-secondary dark:text-gray-400 mb-2">
                {mode === "moodboardToScene"
                  ? "1. Tải Lên Ảnh Moodboard"
                  : "1. Tải Lên Ảnh Không Gian Hoàn Thiện"}
              </label>
              <ImageUpload
            ...
            <div className="space-y-4 flex flex-col h-full">
              <div>
                <label
                  htmlFor="prompt-moodboard"
                  className="block text-sm font-medium text-text-secondary dark:text-gray-400 mb-2"
                >
                  {mode === "moodboardToScene"
                    ? "2. Mô tả loại không gian bạn muốn"
                    : "2. Thêm mô tả/từ khóa (tùy chọn)"}
                </label>
                <textarea
                  id="prompt-moodboard"
                  rows={4}

    ...
    <div className="bg-surface dark:bg-dark-bg p-4 rounded-lg border border-border-color dark:border-gray-700">
      ...
      <div className="space-y-3">
        {prompts.map((prompt, index) => (
          <div
            key={index}
            className="group flex justify-between items-start gap-2 bg-main-bg dark:bg-gray-700/50 p-3 rounded-md"
          >
...
const PromptSuggester: React.FC<PromptSuggesterProps> = ({
  ...
  const handleSelectPrompt = (prompt: string) => {
    if (sourceImage) {
      ...
      onStateChange({
        error: "Không tìm thấy ảnh gốc để gửi đi. Vui lòng tải lại ảnh.",
      });
    }
  };
    ...
    <div className="flex flex-col gap-8">
      <div>
        <h2 className="text-2xl font-bold text-text-primary dark:text-white mb-4">
          AI Gợi ý prompt đồng bộ View
        </h2>
        <p className="text-text-secondary dark:text-gray-300 mb-6">
          Tải lên một bức ảnh kiến trúc, AI sẽ phân tích và đề xuất các ý tưởng
          prompt độc đáo để bạn sử dụng trực tiếp trong tính năng "Đồng Bộ
          View".
        </p>
      </div>
          ...
          <div className="bg-main-bg/50 dark:bg-dark-bg/50 p-6 rounded-xl border border-border-color dark:border-gray-700">
            ...
            <div className="mt-4">
              ...
              <textarea
                ...
                onChange={(e) =>
                  onStateChange({ customInstruction: e.target.value })
                }
                disabled={isLoading}
              />
            </div>
          ...
          <div className="w-full min-h-[400px] bg-surface dark:bg-dark-bg rounded-lg border border-border-color dark:border-gray-700 flex items-center justify-center p-4">
            ...
            {!isLoading && !suggestions && (
              <div className="text-center text-text-secondary dark:text-gray-400 p-4">
                ...
                <p className="mt-2">
                  Tải lên một ảnh và nhấn "Tạo Gợi ý" để xem kết quả.
                </p>
              </div>
            )}
            {!isLoading && suggestions && (

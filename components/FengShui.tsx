...
const FengShui: React.FC<FengShuiProps> = ({ state, onStateChange }) => {
  ...
  // Calculate cost based on "Standard" if image generation is involved
  const getCost = () => {
  ...
  const handleGetLocation = () => {
    if (navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          onStateChange({
            error: `Lỗi lấy vị trí: ${err.message}`,
            isLoading: false,
          });
        },
      );
    ...
    try {
      ...
      if (analysisType === "than-sat") {
        if (!terrainDescription.trim() && !floorPlanImage) {
          throw new Error(
            "Vui lòng mô tả hoặc tải lên ảnh về sát khí bạn nghi ngờ.",
          );
        }

        const basePrompt = `Bạn là một chuyên gia Phong Thủy. Hãy phân tích về Thần Sát (Sát khí) dựa trên thông tin được cung cấp.`;
        const descriptionPart = terrainDescription.trim()
          ? `Mô tả của gia chủ: "${terrainDescription}"`
          : "";
      ...
      // --- HUYEN KHONG ---
        ...
        if (!floorPlanImage)
          throw new Error("Vui lòng tải lên mặt bằng để phân tích.");
      ...
      // --- VAN KHAN ---
        ...
        const vanKhanLabel =
          vanKhanTypes.find((v) => v.value === vanKhanType)?.label || "sự kiện";
      ...
      // --- BAT TRACH / DEFAULT ---
        ...
        if (floorPlanImage) {
          prompt +=
            " Dựa trên mặt bằng được cung cấp, hãy vẽ đè lên ảnh các khu vực Cát/Hung theo Bát Trạch.";
          const result = await processImage(prompt);
          if (result && result.length > 0) {
            onStateChange({
              resultImage: result[0].imageUrl,
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6 bg-surface dark:bg-dark-bg p-6 rounded-xl border border-border-color dark:border-gray-700 h-fit">
                ...
                <div className="flex gap-4">
                  <label className="flex items-center space-x-2 text-text-primary dark:text-white cursor-pointer">
                    <input
                      ...
                      value="male"
                      checked={gender === "male"}
                      onChange={() => onStateChange({ gender: "male" })}
                  ...
                  <label className="flex items-center space-x-2 text-text-primary dark:text-white cursor-pointer">
                    <input
                      ...
                      onChange={() => onStateChange({ gender: "female" })}
                      className="text-accent focus:ring-accent"
                    />
                    <span>Nữ</span>
                  </label>
          ...
          {["bat-trach", "huyen-khong", "duong-trach"].includes(
            analysisType,
          ) && (
            <OptionSelector
              id="house-direction"
          ...
          {["loan-dau", "than-sat"].includes(analysisType) && (
            <div>
              <label className="block text-sm font-medium text-text-secondary dark:text-gray-400 mb-2">
                Mô tả địa thế / Sát khí
              </label>
              <textarea
                rows={4}
                value={terrainDescription}
                onChange={(e) =>
                  onStateChange({ terrainDescription: e.target.value })
                }
                className="w-full bg-main-bg dark:bg-gray-700 border border-border-color dark:border-gray-600 rounded-lg p-3 text-text-primary dark:text-white focus:ring-2 focus:ring-accent outline-none transition-all"
              ...
              {latitude && (
                <p className="text-xs text-green-500 mt-1">
                  Đã lấy tọa độ: {latitude.toFixed(4)}, {longitude?.toFixed(4)}
                </p>
              )}
            </div>
          )}
          ...
          <div>
            <label className="block text-sm font-medium text-text-secondary dark:text-gray-400 mb-2">
              {["loan-dau", "than-sat"].includes(analysisType)
                ? "Ảnh chụp hiện trạng (Tùy chọn)"
                : "Ảnh mặt bằng (Tùy chọn)"}
            </label>
            <ImageUpload
          ...
          <div className="flex items-center justify-between bg-gray-100 dark:bg-gray-800/50 rounded-lg px-4 py-2 border border-gray-200 dark:border-gray-700">
            ...
            {error && (
              <div className="p-3 bg-red-100 border border-red-400 text-red-700 dark:bg-red-900/50 dark:border-red-500 dark:text-red-300 rounded-lg text-sm">
                {error}
              </div>
            )}
          </div>
                ...
                <div className="space-y-6">
                  {resultImage && (
                    <div className="relative rounded-lg overflow-hidden border border-border-color dark:border-gray-600">
                      {floorPlanImage ? (
                        <ImageComparator
                          originalImage={floorPlanImage.objectURL}
                          resultImage={resultImage}
                        />
                      ) : (
                        <img
                          src={resultImage}
                          alt="Feng Shui Result"

...
const MaterialSwapper: React.FC<MaterialSwapperProps> = ({
  state,
  onStateChange,
}) => {
  const {
  ...
  const handleGenerate = async () => {
    ...
    if (!materialImage) {
      onStateChange({
        error: "Vui lòng tải lên ảnh vật liệu hoặc nội thất tham khảo.",
      });
      return;
    }
  ...
  return (
    <div>
      ...
      <h2 className="text-2xl font-bold text-text-primary dark:text-white mb-4">
        AI Thay Vật Liệu / Staging
      </h2>
      <p className="text-text-secondary dark:text-gray-300 mb-6">
        Tải lên ảnh không gian và ảnh vật liệu hoặc đồ nội thất. AI sẽ tự động
        thay thế và staging vào không gian của bạn.
      </p>
        ...
        <div className="space-y-6 bg-main-bg/50 dark:bg-dark-bg/50 p-6 rounded-xl border border-border-color dark:border-gray-700">
          <div>
            <label className="block text-sm font-medium text-text-secondary dark:text-gray-400 mb-2">
              1. Tải Lên Ảnh Không Gian (Phòng, ngoại thất...)
            </label>
            <ImageUpload
          ...
          <div>
            <label className="block text-sm font-medium text-text-secondary dark:text-gray-400 mb-2">
              2. Tải Lên Ảnh Vật Liệu / Nội Thất Tham Khảo
            </label>
            <ImageUpload
              onFileSelect={handleMaterialFileSelect}
              id="material-image-upload"
              previewUrl={materialImage?.objectURL}
            />
          </div>
          <div>
            <label

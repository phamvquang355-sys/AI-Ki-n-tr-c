...
const RulerDisplay: React.FC<RulerDisplayProps> = ({
  rulerData,
  dimension,
  onDimensionChange,
}) => {
  const result: LuBanResult | null = useMemo(
    () => getLuBanResult(dimension, rulerData),
    [dimension, rulerData],
  );
  ...
  const handleInteractionMove = useCallback(
    (clientX: number) => {
      ...
      const currentBase =
        Math.floor((dimension - 1) / rulerData.cycle) * rulerData.cycle;
      ...
      <div className="flex justify-between items-baseline mb-3">
        <div>
          ...
          <p className="text-text-secondary dark:text-gray-400 text-xs">
            {rulerData.description}
          </p>
        </div>
        {result && (
          <div className="text-right">
            <p
              className={`font-bold ${result.isGood ? "text-red-500" : "text-gray-400"}`}
            >
              {result.main}
            </p>
            <p className="text-text-secondary dark:text-gray-400 text-xs">
      ...
      <div className="relative">
        ...
        <div className="flex text-xs text-text-secondary dark:text-gray-400 mb-1">
          {Array.from({ length: 24 }).map((_, i) => (
            <div
              key={i}
              className="flex-1 text-center border-l border-border-color/50 dark:border-gray-600/50 first:border-l-0"
            >
        ...
        >
          ...
          <div className="absolute inset-0 flex">
            {rulerData.categories.map((cat, index) => (
              <div
                key={index}
                className={`flex-1 ${cat.good ? "bg-red-700" : "bg-gray-800"}`}
              ></div>
            ))}
          </div>
          ...
          <div className="absolute inset-0 flex text-white text-[10px] sm:text-xs pointer-events-none">
            {rulerData.categories.map((cat, index) => (
              <div
                key={index}
                className="flex-1 border-r border-white/20 last:border-r-0 flex flex-col items-center justify-between p-1"
              >
...
const LuBanRuler: React.FC<LuBanRulerProps> = ({ state, onStateChange }) => {
  ...
  const dimensionToCheck =
    parseFloat(checkDimension === "width" ? width : height) || 0;

  return (
    <div className="space-y-8">
      <h2 className="text-2xl font-bold text-text-primary dark:text-white text-center">
        Thước Lỗ Ban
      </h2>
      ...
      <div className="max-w-xl mx-auto space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label
              htmlFor="width"
              className="block text-sm text-text-secondary dark:text-gray-400 mb-1"
            >
              Rộng (mm)
            </label>
            <input
      ...
      <div className="space-y-6">
        <RulerDisplay
          rulerData={RULER_52_2_DATA}
          dimension={dimensionToCheck}
          onDimensionChange={handleRulerDrag}
        />
        <RulerDisplay
          rulerData={RULER_42_9_DATA}
          dimension={dimensionToCheck}
          onDimensionChange={handleRulerDrag}
        />
      </div>

...
const Upscale: React.FC<UpscaleProps> = ({ state, onStateChange }) => {
  const { sourceImage, isLoading, error, upscaledImages, numberOfImages } =
    state;
  ...
  const upscalePrompt =
    "Upscale this image to a high resolution. Enhance the details, textures, and lighting to make it look photorealistic and professional. Do not change the composition or the core design.";
    ...
    <div className="flex flex-col gap-8">
      ...
      <div>
        <h2 className="text-2xl font-bold text-text-primary dark:text-white mb-4">
          AI Upscale - Nâng Cấp Chi Tiết
        </h2>
        <p className="text-text-secondary dark:text-gray-300 mb-6">
          Tăng cường độ phân giải và làm sắc nét các chi tiết trong ảnh của bạn.
          Phù hợp để cải thiện chất lượng ảnh render hoặc ảnh chụp.
        </p>
        ...
        <div className="bg-main-bg/50 dark:bg-dark-bg/50 border border-border-color dark:border-gray-700 rounded-xl p-6 flex flex-col items-center">
          <div className="w-full max-w-lg">
            <label className="block text-sm font-medium text-text-secondary dark:text-gray-400 mb-2 text-center">
              1. Tải Lên Ảnh Gốc Cần Nâng Cấp
            </label>
            <ImageUpload
          ...
          <div className="w-full max-w-lg mt-6">
            ...
            {error && (
              <div className="mt-4 p-3 bg-red-100 border border-red-400 text-red-700 dark:bg-red-900/50 dark:border-red-500 dark:text-red-300 rounded-lg text-sm max-w-lg w-full">
                {error}
              </div>
            )}
          </div>
          ...
          <div className="w-full aspect-video bg-main-bg dark:bg-gray-800/50 rounded-lg border-2 border-dashed border-border-color dark:border-gray-700 flex items-center justify-center overflow-hidden">
            ...
            {!isLoading && upscaledImages.length === 0 && (
              <p className="text-text-secondary dark:text-gray-400 text-center p-4">
                {sourceImage
                  ? "Kết quả nâng cấp sẽ hiển thị ở đây."

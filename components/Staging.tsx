...
const Staging: React.FC<StagingProps> = ({ state, onStateChange }) => {
  ...
  const handleAutoPrompt = async () => {
    if (!sceneImage) {
      onStateChange({
        error: "Vui lòng tải ảnh không gian lên trước khi tạo prompt tự động.",
      });
      return;
    }
    ...
    try {
      const generatedPrompt =
        await geminiService.generatePromptFromImageAndText(sceneImage, prompt);
  ...
  const handleGenerate = async () => {
    ...
    if (objectImages.length === 0) {
      onStateChange({
        error: "Vui lòng tải lên ít nhất một ảnh đồ vật cần đặt vào.",
      });
      return;
    }
    ...
    try {
      ...
      // Standard (Flash) Logic
        ...
        results = await geminiService.generateStagingImage(
          fullPrompt,
          sceneImage,
          objectImages,
          numberOfImages,
  ...
  return (
    <div>
      ...
      <p className="text-text-secondary dark:text-gray-300 mb-6">
        Tải lên ảnh không gian và ảnh của một hoặc nhiều đồ vật (nội thất, cây
        cối,...). AI sẽ đặt các đồ vật đó vào không gian một cách tự nhiên theo
        mô tả của bạn.
      </p>
        ...
        <div className="space-y-6 bg-main-bg/50 dark:bg-dark-bg/50 p-6 rounded-xl border border-border-color dark:border-gray-700">
          <div>
            <label className="block text-sm font-medium text-text-secondary dark:text-gray-400 mb-2">
              1. Tải Lên Ảnh Không Gian (Phòng, sân vườn...)
            </label>
            <ImageUpload
              onFileSelect={handleSceneFileSelect}
              id="scene-image-upload-staging"
              previewUrl={sceneImage?.objectURL}
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-text-secondary dark:text-gray-400 mb-2">
              2. Tải Lên Ảnh Các Đồ Vật (Tối đa 12 ảnh)
            </label>
            <MultiImageUpload
              onFilesChange={handleObjectsFileChange}
              maxFiles={12}
            />
          </div>
          <div>
            <label
              htmlFor="prompt-staging"
              className="block text-sm font-medium text-text-secondary dark:text-gray-400 mb-2"
            >
              3. Mô tả vị trí và cách sắp xếp
            </label>
            <textarea
              id="prompt-staging"
              rows={3}

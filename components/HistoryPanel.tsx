...
const HistoryPanel: React.FC = () => {
  ...
  const handleClearHistory = async () => {
    if (
      window.confirm(
        "Bạn có chắc chắn muốn xóa toàn bộ lịch sử không? Hành động này không thể hoàn tác.",
      )
    ) {
      setIsLoading(true);
      try {
  ...
  const handleDeleteItem = async (id: string, e: React.MouseEvent) => {
    ...
    if (window.confirm("Bạn có chắc chắn muốn xóa mục này không?")) {
      ...
      try {
        ...
        alert(
          `Không thể xóa mục này: ${error.message || "Lỗi không xác định"}`,
        );
      } finally {
  ...
  const renderModal = () => {
    ...
    const handleDownload = () => {
      ...
      const isVideo =
        selectedItem.media_type === "video" || !!selectedItem.resultVideoURL;
      ...
      >
        ...
        >
          ...
          <div className="flex-1 flex flex-col w-full lg:max-w-md space-y-5">
            {sourceUrl && (
              <div>
                <h3 className="text-sm font-bold text-text-secondary dark:text-gray-400 mb-2 uppercase tracking-wider">
                  Ảnh Gốc
                </h3>
            ...
            <div className="space-y-4 flex-grow overflow-y-auto pr-2">
              <div>
                <h3 className="text-sm font-bold text-text-secondary dark:text-gray-400 mb-1 uppercase tracking-wider">
                  Công cụ
                </h3>
                <p className="text-text-primary dark:text-white font-medium">
                  {toolDisplayNames[selectedItem.tool] || selectedItem.tool}
                </p>
              </div>
              <div>
                <h3 className="text-sm font-bold text-text-secondary dark:text-gray-400 mb-1 uppercase tracking-wider">
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-text-secondary dark:text-gray-300 text-sm mt-1">
            Xem lại các tác phẩm bạn đã tạo. Dữ liệu được lưu trữ trên đám mây.
          </p>
        </div>
      ...
      {isLoading ? (
        <div className="text-center py-20 bg-surface dark:bg-gray-800/30 rounded-xl border-2 border-dashed border-border-color dark:border-gray-700">
          ...
          <p className="text-text-secondary dark:text-gray-400 animate-pulse">
            Đang tải dữ liệu...
          </p>
        </div>
      ) : history.length === 0 ? (
        <div className="text-center py-20 bg-surface dark:bg-gray-800/30 rounded-xl border-2 border-dashed border-border-color dark:border-gray-700">
          ...
          <p className="text-sm text-text-secondary dark:text-gray-400">
            Hãy bắt đầu sáng tạo để lưu lại các tác phẩm của bạn tại đây.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {history.map((item) => {
            const displayUrl =
              item.media_url || item.resultImageURL || item.resultVideoURL;
            const isVideo =
              item.media_type === "video" || !!item.resultVideoURL;
            ...
            return (
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-3">
                  <p className="text-white text-xs font-bold mb-0.5 truncate">
                    {toolDisplayNames[item.tool] || item.tool}
                  </p>

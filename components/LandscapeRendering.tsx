...
const LandscapeRendering: React.FC<LandscapeRenderingProps> = ({
  state,
  onStateChange,
  onSendToViewSync,
}) => {
  ...
  const updatePrompt = useCallback(
    (
      type: "gardenStyle" | "timeOfDay" | "features",
      newValue: string,
      oldValue: string,
    ) => {
      const getPromptPart = (partType: string, value: string): string => {
        ...
        switch (partType) {
          case "gardenStyle":
            return `theo phong cách ${value}`;
          case "timeOfDay":
            return `vào lúc ${value}`;
          case "features":
            return `và ${value}`;
          default:
            return "";
  ...
  const handleUpscale = async () => {
    ...
    try {
      ...
      const upscalePrompt =
        "Upscale this landscape rendering to a high resolution. Enhance details like plant textures, water reflections, material details, and lighting to make it look photorealistic and professional. Do not change the composition or the core design.";
    ...
    <div className="flex flex-col gap-8">
      ...
      <div>
        <h2 className="text-2xl font-bold text-text-primary dark:text-white mb-4">
          AI Render Sân vườn & Tiểu cảnh
        </h2>
        <p className="text-text-secondary dark:text-gray-300 mb-6">
          Mô tả chi tiết ý tưởng sân vườn của bạn, hoặc tải lên ảnh phác thảo để
          AI trực quan hóa thành một không gian xanh mát, sống động.
        </p>
            ...
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-medium text-text-secondary dark:text-gray-400 mb-2">
                  1. Tải Lên Ảnh Phác Thảo (Tùy chọn)
                </label>
                <ImageUpload
              ...
              <div>
                <label className="block text-sm font-medium text-text-secondary dark:text-gray-400 mb-2">
                  Ảnh Tham Chiếu Phong Cách (Tùy chọn)
                </label>
                <ImageUpload
            ...
            <div className="space-y-4 flex flex-col">
              ...
              <div className="pt-2">
                ...
                <div className="space-y-4">
                  <OptionSelector
                    id="style-selector"
                    label="Phong cách vườn"
                    options={gardenStyleOptions}
                    value={gardenStyle}

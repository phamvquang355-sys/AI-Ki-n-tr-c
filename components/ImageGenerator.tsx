...
const lightingOptions = [
  ...
  {
    value: "buổi tối, đèn vàng từ trong nhà hắt ra, đèn đường sáng",
    label: "Buổi tối",
  },
  {
    value: "đêm khuya, đèn công trình sáng và bầu trời đầy sao",
    label: "Đêm sao",
  },
];
...
const ImageGenerator: React.FC<ImageGeneratorProps> = ({
  ...
  const updatePrompt = useCallback(
    (
      type: "style" | "context" | "lighting" | "weather" | "buildingType",
      newValue: string,
      oldValue: string,
    ) => {
      const getPromptPart = (partType: string, value: string): string => {
        ...
        switch (partType) {
          case "style":
            return `phong cách ${value}`;
          case "context":
            return `bối cảnh ${value}`;
          case "lighting":
            return `ánh sáng ${value}`;
          case "weather":
            return `thời tiết ${value}`;
          case "buildingType":
            return `là một ${value}`;
          default:
            return "";
      ...
      if (oldPart && nextPrompt.includes(oldPart)) {
        nextPrompt = newPart
          ...
          : nextPrompt
              .replace(new RegExp(`,?\\s*${oldPart}`), "")
              .replace(new RegExp(`${oldPart},?\\s*`), "");
      } else if (newPart) {
  ...
  const handleAutoPrompt = async () => {
    if (!sourceImage) {
      onStateChange({
        error: "Vui lòng tải ảnh lên trước khi tạo prompt tự động.",
      });
      return;
    }
    ...
    try {
      const generatedPrompt =
        await geminiService.generatePromptFromImageAndText(
          sourceImage,
          customPrompt,
        );
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if ("Standard" !== "Standard") {
        ...
        imageUrls = await performGeneration(
          customPrompt,
          sourceImage,
          referenceImage,
          numberOfImages,
        ...
        while (attempts < maxAttempts) {
          try {
            ...
            if (apiError.message === "SYSTEM_BUSY") {
              attempts++;
              setStatusMessage(
                `Hệ thống đang bận (${attempts}), vui lòng đợi...`,
              );
      ...
      if (!success) {
        throw new Error(
          "Hệ thống quá tải. Đã hoàn tiền, vui lòng thử lại sau.",
        );
      }
      ...
      if (
        err.message.includes("không đủ credits") ||
  ...
  const handleUpscale = async () => {
    ...
    try {
      ...
      const upscalePrompt =
        "Upscale this architectural rendering to a high resolution. Enhance the details, textures, and lighting to make it look photorealistic and professional. Do not change the composition or the core design.";
  ...
  const handleDownload = () => {
    const url =
      upscaledImage || (resultImages.length > 0 ? resultImages[0] : null);
    if (!url) return;
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="flex flex-col gap-2">
        <h2 className="text-2xl md:text-3xl font-bold text-text-primary dark:text-white">
          AI Render Kiến trúc
        </h2>
        <p className="text-sm md:text-base text-text-secondary dark:text-gray-400">
          Biến phác thảo thành hiện thực hoặc tạo ý tưởng mới từ mô tả văn bản.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
          ...
          <div className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-text-secondary dark:text-gray-400 mb-2">
                1. Tải Lên Ảnh Phác Thảo (Sketch)
              </label>
              <ImageUpload
            ...
            <div>
              ...
              <ImageUpload
                ...
                previewUrl={referenceImage?.objectURL}
              />
            </div>
          </div>
          ...
          <div className="space-y-4 flex flex-col">
            <div className="relative">
              <label
                htmlFor="custom-prompt-architectural"
              ...
              <textarea
                ...
                value={customPrompt}
                onChange={(e) =>
                  onStateChange({ customPrompt: e.target.value })
                }
                disabled={isLoading}
              />
              <button
                ...
                disabled={
                  !sourceImage || isLoading || isUpscaling || isGeneratingPrompt
                }
        ...
        <div className="mt-4">
          <div className="flex items-center justify-between bg-gray-100 dark:bg-gray-800/50 rounded-lg px-4 py-2 mb-3 border border-gray-200 dark:border-gray-700">
            ...
            >
              {isLoading ? (
                <>
                  <Spinner /> {statusMessage || "Đang xử lý..."}
                </>
              ) : (
                "Bắt đầu Render"
              )}
            ...
            {error && (
              <p className="mt-3 text-xs text-red-500 text-center bg-red-50 dark:bg-red-900/20 p-2 rounded border border-red-100 dark:border-red-800">
                {error}
              </p>
            )}
          </div>
        ...
        <div>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
            ...
            {resultImages.length === 1 && (
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button
                  onClick={() =>
                    handleSendImageToSync(upscaledImage || resultImages[0])
                  }
                ...
                <button
                  onClick={() =>
                    setPreviewImage(upscaledImage || resultImages[0])
                  }
                ...
                <button
                  ...
                  className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-1.5 rounded-lg text-sm font-medium shadow-sm transition-colors"
                >
                  Tải xuống
                </button>
              </div>
          ...
          <div className="w-full aspect-[4/3] bg-gray-100 dark:bg-gray-800/50 rounded-2xl border-2 border-dashed border-gray-300 dark:border-gray-700 flex items-center justify-center overflow-hidden relative group">
            ...
            {!isLoading && resultImages.length > 1 && (
              <ResultGrid
                images={resultImages}
                toolName="architecture-render"
                onSendToViewSync={handleSendImageToSync}
              />
            )}
            {!isLoading && resultImages.length === 0 && (
              <div className="text-center p-8 opacity-50">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-16 w-16 md:h-20 md:w-20 mx-auto mb-4 text-gray-300 dark:text-gray-600"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  strokeWidth={1}
                ...
                <p className="text-gray-500 dark:text-gray-400 text-base md:text-lg">
                  Kết quả render sẽ xuất hiện ở đây
                </p>
                <p className="text-xs md:text-sm text-gray-400 dark:text-gray-500">
                  Hãy nhập mô tả hoặc tải ảnh lên để bắt đầu
                </p>
              </div>
            )}
          </div>
